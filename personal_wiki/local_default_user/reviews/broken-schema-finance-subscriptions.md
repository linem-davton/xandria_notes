---
schema_version: 1
title: "Broken Schema: Subscriptions"
page_status: active
aliases:
  - broken schema finance/subscriptions
...
- `missing_frontmatter_key`: Frontmatter is missing required key `finance_area`.
- `missing_frontmatter_key`: Frontmatter is missing required key `state`.
- `missing_frontmatter_key`: Frontmatter is missing required key `cadence`.
- `missing_frontmatter_key`: Frontmatter is missing required key `next_due_on`.
- `missing_frontmatter_key`: Frontmatter is missing required key `currency`.
...
Update `finance/subscriptions` so its frontmatter and typed blocks match the path-derived page family schema.
